...
class PDFBuilder(Builder):
    ...
    def assemble_doctree(self, docname, title, author, appendices):
        ...
        for pendingnode in tree.traverse(addnodes.pending_xref):
            ...
            if pendingnode.get(
                "reftarget", None
            ) == "genindex" and self.config.pdf_use_index:
...
def setup(app):
    ...
    author_texescaped = unicode(app.config.copyright).translate(
        texescape.tex_escape_map
    )
    project_doc_texescaped = unicode(app.config.project + " Documentation").translate(
        texescape.tex_escape_map
    )

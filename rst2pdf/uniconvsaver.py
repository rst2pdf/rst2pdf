...
class PDFDevice:
    ...
    def DrawRectangle(self, start, end):
        self.pdf.rectangle(start.x, start.y, end.x - start.x, end.y - start.y, 1, 0)
...
class PDFGenSaver:
    ...
    def axial_gradient(self, properties, rect):
        ...
        _sketch.fill_axial_gradient(
            image.im, pattern.Gradient().Colors(), 0, border, 0, 200 - border
        )
        self.pdf.saveState()
        apply(self.pdf.transform, trafo.coeff())
        self.pdf.drawInlineImage(
            image, (left - right) / 2, (bottom - top) / 2, right - left, top - bottom
        )
        self.pdf.restoreState()
    ...
    def polybezier(self, paths, properties, bounding_rect, clip=0):
        ...
        if active_fill:
            ...
            method(
                self.make_pdf_path(paths), properties.HasLine(), properties.HasFill()
            )
        # draw the arrows
        if properties.HasLine():
    ...
    def Save(self):
        ...
        for page in self.document.pages:
            count += 1
            app.updateInfo(
                inf2=_("Composing page %u of %u") % (count, pagenum),
                inf3=count * interval
            )
            layers = page + masters
            for layer in layers:
    ...
    def save_objects(self, objects):
        for object in objects:
            if object.is_Compound:
                ...
                self.polybezier(
                    object.Paths(), object.Properties(), object.bounding_rect
                )


def save(document, file, filename, options={}):
    app.updateInfo(
        inf1=_("PDF generation."), inf2=_("Start document composing"), inf3=3
    )

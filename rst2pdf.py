...
def gen_elements(node, depth, in_line_block=False, style=styles['BodyText']):
  ...
  #######################
  ...
  elif isinstance (node, docutils.nodes.option):
    node.elements=[Preformatted(gather_pdftext(node,depth,replaceEnt=False),style=styles['Code'])]
    ...
    if node.parent.get('bullet') or isinstance(node.parent,docutils.nodes.bullet_list):
      # FIXME: use correct bullet symbols, style, check inter-paragraph spacing
      ...
      print node.parent
      sys.exit(1)
    node.elements=[Table([[b,el]],style=tstyles['List'],colWidths=[list_lwidth,None])]
...
if __name__ == "__main__":
  ...
  # tw is the text width. We need it to calculate header-footer height
  tw=pw-lm-rm
